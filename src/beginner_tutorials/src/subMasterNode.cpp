...
void topic_1_Callback(const std_msgs::Int8::ConstPtr& msg_1);
void publish();
...
int main(int argc, char **argv)
{
  ...
  ros::init(argc, argv, "subMasterNode");
  //ros::init(argc, argv, "subMasterNode_1");
  ...
  while (ros::ok())
  {
     ...
     ros::Subscriber topic_1_sup = n_1.subscribe("topic_1", 1000, topic_1_Callback);
...
//int i = 0;
/*  for(std::vector<float>::const_iterator it = msg_1->data.begin(); it != msg_1->data.end(); ++it)
	{
		Arr[i] = *it;
		i++;
	}*/
	
	  ROS_INFO("callbackfn1");
  if((msg_1->data)==49){  //homeing_mode
  check=1;
  }
  if((msg_1->data)==50){  //fk_mode
  check=2;
  }
  if((msg_1->data==51)){  //ik_mode
  check=3;
  }
  ROS_INFO("callbackfn");
  //cin>>check;
